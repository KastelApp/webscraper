...
class RouteHandler {
	 ...
	 * @private
	 */
	private _endpoints: Map<string, Handler> = new Map();
	 ...
	 */
	public append(endpoint: string, handler: Handler): RouteHandler {
		this._endpoints.set(endpoint, handler);
	...
	public async handle(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
		...
		const [res, error] = await this.promiseHandler(handler(this));
